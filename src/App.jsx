...
// You mentioned three files already sitting in public/images — plasmid.jpg
// and the mini_magick... one are still unused below; drop either path into
// whichever card image you want them on.
const HOME_CARDS = [
  {
    key: "about",
    title: "About GFPR",
...
// ============================================================================
// Points at the TEM plasmid micrograph you placed in public/images. Swap the
// filename for either of your other two images if you'd rather use those —
// both work the same way, just drop the file in `public/images/` and change
// the path below (works after `npm run dev`).
...
// ---- 9.4 World sample map (real basemap, zoomable, clickable) --------------
// Basemap: "BlankMap-Equirectangular.svg" (Natural Earth data, via Wikimedia
// Commons, CC0 / public domain — https://commons.wikimedia.org/wiki/File:
...
const SAMPLE_POINTS = [
  { label: "Italy", ...zip(toMapXY(42.0, 12.5)), count: 420, category: "dairy" },
  { label: "Türkiye", ...zip(toMapXY(39.0, 35.0)), count: 260, category: "ferm_bev" },
  { label: "France", ...zip(toMapXY(46.5, 2.2)), count: 310, category: "dairy" },
  { label: "Germany", ...zip(toMapXY(51.0, 10.5)), count: 180, category: "ferm_grains" },
  { label: "United Kingdom", ...zip(toMapXY(54.0, -2.0)), count: 120, category: "other" },
  { label: "Netherlands", ...zip(toMapXY(52.3, 5.5)), count: 90, category: "dairy" },
  { label: "Poland", ...zip(toMapXY(52.0, 19.5)), count: 100, category: "fruit_veg" },
  { label: "USA", ...zip(toMapXY(39.0, -98.0)), count: 480, category: "meat" },
  { label: "Mexico", ...zip(toMapXY(23.0, -102.0)), count: 150, category: "ferm_bev" },
  { label: "Brazil", ...zip(toMapXY(-10.0, -55.0)), count: 200, category: "meat" },
  { label: "China", ...zip(toMapXY(35.0, 103.0)), count: 350, category: "fruit_veg" },
  { label: "Japan", ...zip(toMapXY(36.0, 138.0)), count: 300, category: "ferm_bev" },
  { label: "South Korea", ...zip(toMapXY(36.0, 127.7)), count: 240, category: "fruit_veg" },
  { label: "India", ...zip(toMapXY(22.0, 79.0)), count: 220, category: "dairy" },
  { label: "Thailand", ...zip(toMapXY(15.0, 101.0)), count: 160, category: "seafood" },
  { label: "Vietnam", ...zip(toMapXY(16.0, 106.0)), count: 140, category: "seafood" },
  { label: "Egypt", ...zip(toMapXY(26.8, 30.8)), count: 90, category: "other" },
  { label: "Nigeria", ...zip(toMapXY(9.1, 8.7)), count: 70, category: "ferm_grains" },
  { label: "Kenya", ...zip(toMapXY(-1.0, 37.9)), count: 60, category: "dairy" },
  { label: "Australia", ...zip(toMapXY(-25.0, 134.0)), count: 130, category: "meat" },
];
...
function WorldHeatMap({ onSeeSamples }) {
  ...
  const [pan, setPan] = useState({ x: 0, y: 0 }); // top-left of the view rect, in map units
  ...
  return (
    <div className="rounded-2xl p-5" style={{ backgroundColor: "#fff", border: `1px solid ${COLORS.line}` }}>
      ...
      >
        {/* faint ocean tint behind the basemap so land reads clearly */}
      ...
      <div className="mt-2 min-h-[40px] flex items-center justify-between flex-wrap gap-2">
        {active ? (
          ...
          <span className="text-xs" style={{ color: COLORS.inkSoft }}>
            ...
            country. Basemap: Natural Earth via Wikimedia Commons (public domain).
          </span>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ backgroundColor: COLORS.paper }}>
      <section className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div
          className="max-w-3xl mx-auto mt-10 space-y-4 text-[15px] leading-relaxed"
          style={{ color: COLORS.ink, fontFamily: FONT_BODY, textAlign: "justify" }}
        >
          <p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-5 mt-2" style={{ textAlign: "left" }}>
...
// ---- 10.1 A "Label: value" row for the sample-detail info list ------------
...
${wrapSequence(seq)}`}
            ...
            {/* General Info + Annotation Summary, one card, plain "Label: value" lines */}
            <div className="rounded-2xl p-5" style={{ backgroundColor: "#fff", border: `1px solid ${COLORS.line}` }}>
              ...
              <div className="space-y-1.5">
                ...
                <InfoRow label="Category">{catLabel(record.category)} → {record.sampleType} → {record.subtype}</InfoRow>
                <InfoRow label="Fermented">{fermented ? "Yes" : "No"}</InfoRow>
                <InfoRow label="Host"><em>{record.host}</em></InfoRow>
                <InfoRow label="Plasmid Size">{record.sizeKb} kb</InfoRow>
                    ...
                    return (
                      <div key={a.key}>
                        <div className="text-sm" style={{ fontWeight: 700, color: COLORS.ink, fontFamily: FONT_BODY }}>
                          {a.short} <span style={{ fontWeight: 400, color: COLORS.inkSoft }}>({hits.length} hit{hits.length > 1 ? "s" : ""})</span>
                        </div>
